...
// model code...it is a bit redundant in this case...we could just call getJSON directly...
// but if our model became more complex this sets us up to accomodate that.
...
function renderShipList(ships, shipListElement) {
    ...
    // that table and it has 2 children: thead and tbody...we need to put our ships into 
    // tbody...so I reference the second child.
...
async function showShips(url = 'https://swapi.co/api/starships/') {
    ...
    if (results.next) {
        ...
        // normally we would prefer the addEventListener method of adding a listener. Using something 
        // like 'element.onEvent = event_function' has the limitation of only being able to hold one 
        // listener of the type. In this case that is a good thing however. Because we are not re-creating 
        // the buttons each time we load a new batch of data we could end up with several listeners attached 
    ...
    if (results.previous) {
        ...
        // we need to set the once option on the listener since the buttons do not get recreated with 
        //each page load. If we don't we will end up with several listeners all trying to fire at once 
        // after a few pages
        prev.ontouchend = () => {
