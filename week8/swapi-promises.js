...
// getJSON directly...but if our model became more complex this sets us up to accomodate that.
function getShips(url) {
    return getJSON(url);
}
//  View code
function renderShipList(ships, shipListElement) {
    // I decided to use a table to display my list of ships. The shipList 
    // Element is that table and it has 2 children: thead and tbody...we 
    // need to put our ships into tbody...so I reference the second child.
...
function showShips(url = 'https://swapi.co/api/starships/') {
    getShips(url).then(function (data) {
        ...
        if (data.next) {
            ...
            // Using something like 'element.onEvent = event_function' has the limitation of 
            // only being able to hold one listener of the type we choose. In this case that 
            // is a good thing however. Because we are not re-creating the buttons each time we 
            // load a new batch of data we could end up with several listeners attached to each 
